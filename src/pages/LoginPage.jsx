        ...
        pollRef.current = setInterval(async () => {
                try {
                          ...
                          if (data.authenticated && data.user) {
                                      ...
                                      handleTelegramLogin(null, data.user, data.isNew); setStep('done')
                          }
        ...
        <div className="login-page">
              <div className="login-card">
                      <div className="login-logo">
                                ...
                                <h1>호치민방앗간</h1>h1>
                                <p className="logo-sub">베트남 밤문화 No.1 커뮤니티</p>p>
                      </div>div>
              
                {step === 'idle' && (
                    <>
                                <div className="login-divider"><span>텔레그램 봇으로 간편 로그인</span>span></div>div>
                      {error && <p className="login-error">{error}</p>p>}
                                <button className="btn-bot-login" onClick={startLogin}>
                                              📱 텔레그램으로 로그인
                                </button>button>
                                <div className="login-benefits">
                                              <div className="benefit-item"><span className="benefit-icon">+</span>span><span>텔레그램 계정으로 1초 로그인</span>span></div>div>
                                              <div className="benefit-item"><span className="benefit-icon gold">+</span>span><span>가입 즉시 <strong>10 포인트</strong>strong> 지급</span>span></div>div>
                                              <div className="benefit-item"><span className="benefit-icon">+</span>span><span>계정 전환도 간편하게</span>span></div>div>
                                </div>div>
                    </>>
                  )}
              
                {step === 'loading' && (
                    <div className="login-loading"><p>인증 코드 생성 중...</p>p></div>div>
                      )}
              
                {step === 'waiting' && (
                    <div className="login-waiting">
                                <p className="waiting-title">아래 버튼을 눌러 봇에서 인증하세요</p>p>
                                <div className="token-box">
                                              <span className="token-label">인증 코드</span>span>
                                              <span className="token-val">{token}</span>span>
                                </div>div>
                                <a href={botLink} target="_blank" rel="noopener noreferrer" className="btn-open-bot">
                                              📲 텔레그램 봇 열기
                                </a>a>
                                <p className="waiting-hint">봇에서 인증 완료 후 자동으로 로그인됩니다</p>p>
                                <p className="waiting-timer">남은 시간: {min}:{sec}</p>p>
                                <button className="btn-cancel" onClick={cancel}>취소</button>button>
                    </div>div>
                      )}
              
                {step === 'done' && (
                    <div className="login-done"><p>로그인 완료! 이동 중...</p>p></div>div>
                      )}
              </div>div>
        </div>div>
      )
