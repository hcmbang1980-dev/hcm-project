...
const SECRET_ALLOWED = ['free', 'review', 'qna', 'intro']
...
export default function WritePage() {
  ...
  const allowSecret = SECRET_ALLOWED.includes(type)
  ...
  const handleSubmit = async () => {
    ...
    const { data, error } = await supabase.from('posts').insert({
      ...
      is_secret: allowSecret ? isSecret : false,
      deleted: false,
    }).select().single()
    if (!error && data) {
      ...
      if (pointType) {
        ...
        await supabase.from('user_activity_logs').insert({ user_id: user.id, action: '게시글작성', target_id: String(data.id) })
  ...
  const pointType = BOARD_POINT_TYPE[type]
  const pointAmount = pointType === 'review' ? Number(settings.review_point || 20)
    : pointType === 'post' ? Number(settings.post_point || 5) : 0
    ...
    <div style={{ paddingTop:'84px', minHeight:'100vh' }}>
      <div className="container" style={{ maxWidth:'800px', padding:'30px 20px' }}>
        ...
        <div className="card" style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            <div>
              ...
              {imageUrl && (
                <img src={imageUrl} alt="preview"
                  style={{ marginTop:'8px', maxWidth:'100%', maxHeight:'200px', borderRadius:'8px', objectFit:'cover' }}
                  onError={e => { e.target.style.display = 'none' }} />
              )}
            </div>
          )}
          <textarea
            placeholder={"내용을 입력하세요..." + (pointAmount > 0 ? " (+"+pointAmount+"P)" : "")}
            value={content}
          ...
          />
          {allowSecret && (
            <label style={{ display:'flex', alignItems:'center', gap:'10px', cursor:'pointer', userSelect:'none', color: isSecret ? '#d4af37' : '#888', fontSize:'14px' }}>
              <input
                type="checkbox"
                checked={isSecret}
          ...
          <div style={{ display:'flex', gap:'12px', justifyContent:'flex-end' }}>
            <button onClick={() => navigate(-1)} style={{ padding:'10px 20px', background:'none', border:'1px solid #333', borderRadius:'8px', color:'#888', cursor:'pointer' }}>
              취소
            </button>
