...
// EXP 시스템 (AttendancePage와 동일하게 맞춤)
const EXP_TABLE = (() => {
  const t = [0]
  for (let lv = 1; lv <= 150; lv++) {
    let need
    if (lv < 30) {
      need = 20 + Math.floor(lv * 3)
    } else if (lv < 60) {
      need = Math.floor(110 + (lv - 30) * 15)
    } else if (lv < 100) {
      need = Math.floor(560 + (lv - 60) * 50)
    } else {
      need = Math.floor(2560 + (lv - 100) * 200)
    }
    t.push(t[lv - 1] + need)
  }
  return t
...
function getExpInfo(exp) {
  ...
  const progress = nextFloor
    ? Math.min(100, ((exp - currentFloor) / (nextFloor - currentFloor)) * 100)
    : 100
...
export default function MyPage() {
  ...
  const fetchData = async () => {
    const [userData, levelData, attData, historyData, postsData] = await Promise.all([
      ...
      <div className="container">
        ...
        <div className="mypage-level-card card">
          <div className="mypage-grade-row">
            <div className="mypage-grade-icon">{grade.icon}</div>
            <div className="mypage-grade-info">
              <span className="mypage-grade-name">{grade.grade}</span>
              <span className="mypage-level-num">Lv.{level}</span>
            </div>
            <div className="mypage-att-stats">
              <div className="mypage-stat">
                <span className="mypage-stat-val">{streak}</span>
                <span className="mypage-stat-lbl">연속 출석</span>
              </div>
              <div className="mypage-stat">
                <span className="mypage-stat-val">{totalDays}</span>
                <span className="mypage-stat-lbl">총 출석일</span>
              </div>
            </div>
          </div>
          ...
          <div className="mypage-exp-bar-area">
            <div className="mypage-exp-labels">
              <span>{userExp.toLocaleString()} EXP</span>
              <span>{nextFloor ? nextFloor.toLocaleString() + ' EXP' : 'MAX'}</span>
            </div>
            <div className="mypage-exp-track">
              <div className="mypage-exp-fill" style={{ width: progress + '%' }}></div>
            </div>
        ...
        <div className="card" style={{ marginTop: '16px' }}>
          <h3 className="card-title">🏅 등급 현황</h3>
          <div className="mypage-grade-list">
            {GRADE_INFO.map(g => (
              <div key={g.grade} className={"mypage-grade-item" + (grade.grade === g.grade ? ' active' : '')}>
                <span className="mypage-grade-emoji">{g.icon}</span>
                <span className="mypage-grade-label">{g.grade}</span>
                <span className="mypage-grade-range">Lv.{g.minLv}~{g.maxLv}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card" style={{marginTop:'20px'}}>
          ...
          {history.length === 0 ? <p className="empty-text">포인트 내역이 없습니다.</p> : history.map(h => (
            <div key={h.id} className="history-item">
              ...
              <span className="history-date">{new Date(h.created_at).toLocaleDateString('ko-KR')}</span>
            </div>
          ))}
        </div>
