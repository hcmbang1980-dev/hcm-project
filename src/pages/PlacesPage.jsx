...
.from('places')
.select('id,name,category,address,phone,description,price_range,banner_url,views,likes,status')
...
{places.map(p=>(
<div key={p.id} style={{...S.card,padding:0,overflow:'hidden'}} onClick={()=>navigate('/place/'+p.id)}>
{p.banner_url
? <img src={p.banner_url} alt={p.name} style={{width:'100%',height:160,objectFit:'cover'}} />
: <div style={{width:'100%',height:160,background:'#222',display:'flex',alignItems:'center',justifyContent:'center',fontSize:40}}>
{category==='karaoke'?'🎤':category==='club'?'🍺':category==='massage'?'💆':category==='adult'?'💋':category==='villa'?'🏠':category==='rent'?'🚗':'🍜'}
</div>}
<div style={{padding:16}}>
<h3 style={{color:'#fff',fontWeight:'bold',marginBottom:6,fontSize:16}}>{p.name}</h3>
{p.address && <p style={{color:'#888',fontSize:13,marginBottom:4}}>📍 {p.address}</p>}
{p.phone && <p style={{color:'#888',fontSize:13,marginBottom:4}}>📞 {p.phone}</p>}
{p.price_range && <p style={{color:'#d4af37',fontSize:13,marginBottom:4}}>💰 {p.price_range}</p>}
{p.description && <p style={{color:'#aaa',fontSize:13,marginTop:8,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{p.description}</p>}
<div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:12}}>
<span style={{color:'#555',fontSize:12}}>👁 {p.views||0}</span>
<span style={{color:'#d4af37',fontSize:13,fontWeight:'bold'}}>상세보기 →</span>
</div>
</div>
</div>
))}
