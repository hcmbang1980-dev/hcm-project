...
export default function AdminPage() {
  ...
  const fetchUsers = async () => {
    const { data, error } = await supabase.from('users').select('id, nickname, telegram_id, role, level, created_at').order('created_at', { ascending: false })
  ...
  const saveSiteStats = async () => {
    ...
    const { error } = await supabase.from('site_stats').upsert(updateData, { onConflict: 'id' })
    ...
    <div className="admin-page">
      ...
      <div className="admin-content">
        ...
        {activeTab === 'ui' && (
          <div>
            <h2>🎨 UI 설정</h2>
            <p style={{ color:'#888', marginBottom:20 }}>메인화면 레이아웃, 채팅창 위치 등 사이트 UI를 어드민에서 직접 제어합니다.</p>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
              <div style={{ ...S.card }}>
                <h3 style={{ color:'#d4af37', marginBottom:16 }}>💬 채팅창 설정</h3>
                <label style={{ color:'#aaa', fontSize:13, display:'block', marginBottom:8 }}>채팅창 표시</label>
                ...
                <label style={{ color:'#aaa', fontSize:13, display:'block', marginBottom:8 }}>채팅창 위치</label>
                ...
                {uiSettings.chat_position !== 'inline' && (
                  <p style={{ color:'#666', fontSize:11, marginBottom:12 }}>💡 팝업 모드: 드래그·크기조절·최소화 지원</p>
                )}
                {uiSettings.chat_position !== 'inline' && (
            ...
            <div style={{ ...S.card, marginBottom:20 }}>
              ...
              <p style={{ color:'#666', fontSize:12 }}>예: hero,stats,notice,board,banner</p>
            </div>
