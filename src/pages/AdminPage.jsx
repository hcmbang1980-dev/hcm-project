  ...
  <div style={{ minHeight:'100vh', background:'#0a0a0a', display:'flex', alignItems:'center', justifyContent:'center' }}>
    <div style={{ background:'#1a1a1a', border:'2px solid #d4af37', borderRadius:16, padding:'48px 40px', width:'100%', maxWidth:380, boxShadow:'0 20px 60px rgba(0,0,0,0.8)' }}>
      <div style={{ textAlign:'center', marginBottom:32 }}>
        <div style={{ fontSize:48, marginBottom:12 }}>🔐</div>div>
        <h1 style={{ color:'#d4af37', fontSize:22, fontWeight:'bold', marginBottom:8 }}>관리자 인증</h1>h1>
        <p style={{ color:'#666', fontSize:14 }}>관리자 PIN을 입력하세요</p>p>
      </div>div>
      <form onSubmit={handleSubmit}>
        ...
        {error && <div style={{ background:'#2a1010', border:'1px solid #ff4444', borderRadius:8, padding:'10px 14px', color:'#ff6666', fontSize:13, marginBottom:16, textAlign:'center' }}>{error}</div>div>}
        {locked && <div style={{ color:'#ff9944', textAlign:'center', marginBottom:12, fontSize:14 }}>잠금 해제까지: {Math.floor(lockTimer/60)}:{String(lockTimer%60).padStart(2,'0')}</div>div>}
...
const [uiSettings, setUiSettings] = useState({
  chat_enabled: 'true', chat_position: 'inline',
  chat_right: '20', chat_bottom: '20', chat_width: '380', chat_height: '500',
  stats_visible: 'true', notice_visible: 'true', banner_visible: 'true',
  hero_layout: 'default', main_sections_order: 'hero,stats,notice,board,banner',
})
...
const saveUserChanges = async (u) => {
  ...
  try {
    ...
    .from('users')
    .update({ role: u.role, level: u.level, level_num: levelNum })
    .eq('id', u.id)
...
const S = { input: { width:'100%', padding:'8px 12px', background:'#222', border:'1px solid #444', color:'#fff', borderRadius:6, boxSizing:'border-box' }, card: { background:'#1a1a1a', padding:20, borderRadius:10, border:'1px solid #333' } }
...
return (
  ...
  <div className="admin-content">
    ...
    <tr style={{ borderBottom:'2px solid #d4af37' }}>
      ...
      <th key={h} style={{ padding:10, textAlign:'left', color:'#d4af37' }}>{h}</th>th>
      ))}
    </tr>tr>
    </thead>thead>
    <tbody>
      ...
      {filteredUsers.length === 0 && <p style={{ color:'#666', textAlign:'center', padding:40 }}>회원이 없습니다.</p>p>}
    </div>div>
  )}
    ...
    <div>
    <h2>📊 통계 수치 관리</h2>h2>
    <p style={{ color:'#888', marginBottom:20 }}>기본 수치와 표시 텍스트를 설정하세요</p>p>
    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:20 }}>
      ...
      ['total_label','total_base','누적방문자수'],
        ...
        <label style={{ color:'#888', fontSize:12, display:'block', marginBottom:6 }}>표시 텍스트</label>label>
        <input value={siteStats[lk]||''} onChange={e => setSiteStats(p => ({...p,[lk]:e.target.value}))} placeholder={ph}
          style={{ ...S.input, marginBottom:8 }} />
        <label style={{ color:'#888', fontSize:12, display:'block', marginBottom:6 }}>기본 수치</label>label>
        <input type="number" value={siteStats[bk]||0} onChange={e => setSiteStats(p => ({...p,[bk]:e.target.value}))}
          style={{ ...S.input, color:'#d4af37' }} />
        </div>div>
        ))}
    </div>div>
    <button onClick={saveSiteStats}
      style={{ background:'#d4af37', color:'#000', border:'none', padding:'12px 24px', borderRadius:8, fontWeight:'bold', cursor:'pointer', fontSize:15 }}>
    💾 저장
    </button>button>
    </div>div>
  )}
    ...
    <h3 style={{ color:'#d4af37', marginBottom:16 }}>새 팝업 추가</h3>h3>
    ...
    <textarea value={newPopup.content} onChange={e => setNewPopup(p => ({...p,content:e.target.value}))} placeholder="팝업 내용" rows={4}
    ...
    <button onClick={addPopup} style={{ background:'#d4af37', color:'#000', border:'none', padding:'10px 20px', borderRadius:8, fontWeight:'bold', cursor:'pointer' }}>+ 팝업 추가</button>button>
    </div>div>
      {popups.map(p => (
      <div key={p.id} style={{ ...S.card, marginBottom:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
      <div>
      <div style={{ fontWeight:'bold', color:'#fff', marginBottom:4 }}>{p.title}</div>div>
      <div style={{ color:'#888', fontSize:13 }}>{(p.content||'').substring(0,60)}</div>div>
      </div>div>
      <div style={{ display:'flex', gap:8 }}>
      <button onClick={() => togglePopup(p.id,p.is_active)} style={{ background: p.is_active ? '#22c55e' : '#555', color:'#fff', border:'none', padding:'6px 14px', borderRadius:6, cursor:'pointer', fontSize:13 }}>{p.is_active ? '활성' : '비활성'}</button>button>
      <button onClick={() => deletePopup(p.id)} style={{ background:'#ff4444', color:'#fff', border:'none', padding:'6px 14px', borderRadius:6, cursor:'pointer', fontSize:13 }}>삭제</button>button>
      </div>div>
      </div>div>
      ))}
      {popups.length === 0 && <p style={{ color:'#888', textAlign:'center', padding:40 }}>등록된 팝업이 없습니다.</p>p>}
    </div>div>
  )}
    ...
    <h2>📢 광고 관리</h2>h2>
    <div style={{ ...S.card, marginBottom:24 }}>
    <h3 style={{ color:'#d4af37', marginBottom:16 }}>새 광고 추가</h3>h3>
    ...
    <option value="top">상단</option>option><option value="bottom">하단</option>option><option value="sidebar">사이드바</option>option>
    </select>select>
    <button onClick={addAd} style={{ background:'#d4af37', color:'#000', border:'none', padding:'10px 20px', borderRadius:8, fontWeight:'bold', cursor:'pointer' }}>+ 광고 추가</button>button>
    </div>div>
      {ads.map(a => (
      <div key={a.id} style={{ ...S.card, marginBottom:12, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
      <div>
      <div style={{ fontWeight:'bold', color:'#fff', marginBottom:4 }}>{a.title}</div>div>
      <div style={{ color:'#888', fontSize:13 }}>{a.position} | {a.link_url}</div>div>
      </div>div>
      <div style={{ display:'flex', gap:8 }}>
      <button onClick={() => toggleAd(a.id,a.is_active)} style={{ background: a.is_active ? '#22c55e' : '#555', color:'#fff', border:'none', padding:'6px 14px', borderRadius:6, cursor:'pointer', fontSize:13 }}>{a.is_active ? '활성' : '비활성'}</button>button>
      <button onClick={() => deleteAd(a.id)} style={{ background:'#ff4444', color:'#fff', border:'none', padding:'6px 14px', borderRadius:6, cursor:'pointer', fontSize:13 }}>삭제</button>button>
      </div>div>
      </div>div>
      ))}
      {ads.length === 0 && <p style={{ color:'#888', textAlign:'center', padding:40 }}>등록된 광고가 없습니다.</p>p>}
    </div>div>
  )}
    ...
    <div>
    <h2>🎨 UI 설정</h2>h2>
    <p style={{ color:'#888', marginBottom:20 }}>메인화면 레이아웃, 채팅창 위치 등 사이트 UI를 어드민에서 직접 제어합니다.</p>p>
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
    <div style={{ ...S.card }}>
    <h3 style={{ color:'#d4af37', marginBottom:16 }}>💬 채팅창 설정</h3>h3>
    <label style={{ color:'#aaa', fontSize:13, display:'block', marginBottom:8 }}>채팅창 표시</label>label>
    ...
    <label style={{ color:'#aaa', fontSize:13, display:'block', marginBottom:8 }}>채팅창 위치</label>label>
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
      ...
      <p style={{ color:'#666', fontSize:11, marginBottom:12 }}>💡 팝업 모드: 드래그·크기조절·최소화 지원</p>p>
    )}
      {uiSettings.chat_position !== 'inline' && (
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
        {[['chat_right','우측/좌측 여백(px)'],['chat_bottom','상단/하단 여백(px)'],['chat_width','기본 가로 크기(px)'],['chat_height','기본 세로 크기(px)']].map(([key,label]) => (
        <div key={key}>
        <label style={{ color:'#aaa', fontSize:12, display:'block', marginBottom:4 }}>{label}</label>label>
        <input type="number" value={uiSettings[key]||0} onChange={e => setUiSettings(s => ({...s,[key]:e.target.value}))}
          style={{ ...S.input, color:'#d4af37' }} />
        </div>div>
        ))}
      </div>div>
    ...
    <div style={{ ...S.card }}>
    <h3 style={{ color:'#d4af37', marginBottom:16 }}>🏠 메인화면 섹션</h3>h3>
      ...
      <div style={{ display:'flex', gap:6 }}>
        ...
        <h3 style={{ color:'#d4af37', marginBottom:12 }}>📋 메인 섹션 순서 (쉼표로 구분)</h3>h3>
        <input value={uiSettings.main_sections_order||''} onChange={e => setUiSettings(s => ({...s,main_sections_order:e.target.value}))}
          placeholder="hero,stats,notice,board,banner" style={{ ...S.input, marginBottom:8 }} />
        <p style={{ color:'#666', fontSize:12 }}>예: hero,stats,notice,board,banner</p>p>
        </div>div>
    <button onClick={saveUiSettings}
      style={{ background:'#d4af37', color:'#000', border:'none', padding:'12px 24px', borderRadius:8, fontWeight:'bold', cursor:'pointer', fontSize:15 }}>
    💾 UI 설정 저장
    </button>button>
    </div>div>
  ...
  const [role, setRole] = useState(u.role || 'user')
    const [level, setLevel] = useState(u.level || '새싹')
      ...
      return (
        ...
        <td style={{ padding:10, color:'#fff', minWidth:100, whiteSpace:'nowrap' }}>{u.nickname || '-'}</td>td>
        <td style={{ padding:10, color:'#888' }}>@{u.telegram_id || '-'}</td>td>
        <td style={{ padding:10 }}>
        <select value={role} onChange={e => setRole(e.target.value)}
          style={{ background:'#222', color: ROLE_COLORS[role]||'#fff', border:'1px solid #444', padding:'4px 8px', borderRadius:4, fontWeight:'bold' }}>
        ...
        <select value={level} onChange={e => setLevel(e.target.value)}
          style={{ background:'#222', color:'#d4af37', border:'1px solid #444', padding:'4px 8px', borderRadius:4 }}>
          {LEVEL_OPTIONS.map(lv => <option key={lv} value={lv}>{lv}</option>option>)}
        </select>select>
        </td>td>
