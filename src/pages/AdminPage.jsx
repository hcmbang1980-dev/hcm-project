...
const [newPlaceImage, setNewPlaceImage] = useState({image_url:'',caption:''})
...
const togglePlaceStatus = async (p) => {
const next = p.status==='노출중' ? '비공개' : '노출중'
const {error} = await supabase.from('places').update({status:next}).eq('id',p.id)
...
<label style={{color:'#888',fontSize:12}}>상세 설명 (영업시간, 서비스 등)</label>
<label style={{background:'#333',color:'#d4af37',border:'1px solid #d4af37',padding:'4px 10px',borderRadius:6,fontSize:12,cursor:'pointer'}}>
🖼 본문에 사진 추가
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
<div>
<label style={{color:'#888',fontSize:12,display:'block',marginBottom:4}}>이미지 (PC/기본)</label>
<ImageInput value={newPopup.image_url} onChange={url=>setNewPopup(p=>({...p,image_url:url}))} placeholder="이미지 URL 또는 업로드" uploadImage={uploadImage} />
</div>
<div>
<label style={{color:'#888',fontSize:12,display:'block',marginBottom:4}}>모바일 전용 이미지 (선택)</label>
<ImageInput value={newPopup.mobile_image_url} onChange={url=>setNewPopup(p=>({...p,mobile_image_url:url}))} placeholder="모바일 이미지 URL 또는 업로드" uploadImage={uploadImage} />
</div>
</div>
...
<div>
<label style={{color:'#888',fontSize:12,display:'block',marginBottom:4}}>이미지 (PC/기본)</label>
<ImageInput value={newAd.image_url} onChange={url=>setNewAd(a=>({...a,image_url:url}))} placeholder="이미지 URL 또는 업로드" uploadImage={uploadImage} />
</div>
<div>
<label style={{color:'#888',fontSize:12,display:'block',marginBottom:4}}>모바일 전용 이미지 (선택)</label>
<ImageInput value={newAd.mobile_image_url} onChange={url=>setNewAd(a=>({...a,mobile_image_url:url}))} placeholder="모바일 이미지 URL 또는 업로드" uploadImage={uploadImage} />
</div>
</div>
...
function ImageInput({ value, onChange, placeholder, uploadImage }) {
...
style={{flex:1,padding:'8px 12px',background:'#222',border:'1px solid #444',color:'#fff',borderRadius:6,boxSizing:'border-box',minWidth:0}} />
...
style={{background:uploading?'#555':'#333',color:'#d4af37',border:'1px solid #d4af37',padding:'8px 12px',borderRadius:6,cursor:uploading?'not-allowed':'pointer',fontSize:12,whiteSpace:'nowrap',flexShrink:0}}>
{uploading?'업로드중...':'📁 업로드'}
</button>
{value && <img src={value} alt="" style={{width:36,height:36,objectFit:'cover',borderRadius:6,flexShrink:0}} />}
</div>
)
}
