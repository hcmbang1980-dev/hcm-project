...
export default function Header() {
  ...
  return (
    <header className="header">
      ...
      {showTgLogin && (
        <div className="tg-login-modal" onClick={() => setShowTgLogin(false)}>
          <div className="tg-login-box" onClick={e => e.stopPropagation()}>
            ...
            <div id="tg-widget-container">
              <script async src="https://telegram.org/js/telegram-widget.js?22"
                data-telegram-login={import.meta.env.VITE_TELEGRAM_BOT_NAME || 'bangasgan_bot'}
                data-size="large"
                data-onauth="onTelegramAuth(user)"
                data-request-access="write">
              </script>
            </div>
            <button className="btn-close" onClick={() => setShowTgLogin(false)}>✕ 닫기</button>
          </div>
        </div>
      )}
