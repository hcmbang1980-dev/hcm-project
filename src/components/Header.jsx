...
export default function Header() {
  ...
  const userLevel = user?.level || '새싹'
  const levelIcon = LEVEL_ICONS[userLevel] || '🌱'

  const tgModal = !user && showTgLogin ? createPortal(
    React.createElement('div', { className: 'tg-login-overlay', onClick: handleCloseModal },
      React.createElement('div', { className: 'tg-login-popup', onClick: e => e.stopPropagation() },
        React.createElement('h2', { className: 'gold-text' }, '텔레그램으로 로그인'),
        React.createElement('p', null, '텔레그램 계정으로 간편하게 로그인하세요'),
        React.createElement('div', { ref: widgetRef, style: { margin:'16px 0', minHeight:'60px', display:'flex', justifyContent:'center', alignItems:'center' } }),
        React.createElement('button', { className: 'btn-close', onClick: handleCloseModal }, '닫기')
      )
    ),
    document.body
  ) : null
            ...
            {user ? (
              <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                <div ref={notifRef} style={{ position:'relative' }}>
                  <button onClick={() => setNotifOpen(v => !v)} style={{ background:'none', border:'none', cursor:'pointer', position:'relative', fontSize:'20px', lineHeight:1, padding:'4px' }} title="알림">
                    🔔
                    {unreadCount > 0 && (
                      <span style={{ position:'absolute', top:'-4px', right:'-4px', background:'#ff4444', color:'#fff', borderRadius:'50%', width:'18px', height:'18px', fontSize:'11px', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:'bold' }}>
                        {unreadCount > 9 ? '9+' : unreadCount}
                      </span>
                    )}
                  </button>
                  {notifOpen && (
                    <div style={{ position:'absolute', right:0, top:'calc(100% + 8px)', background:'#1a1a1a', border:'1px solid #333', borderRadius:'12px', width:'300px', maxHeight:'400px', overflow:'auto', zIndex:1000, boxShadow:'0 8px 32px rgba(0,0,0,0.6)' }}>
                      <div style={{ padding:'12px 16px', borderBottom:'1px solid #333', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        ...
                        {unreadCount > 0 && <button onClick={markAllAsRead} style={{ background:'none', border:'none', color:'#888', fontSize:'12px', cursor:'pointer' }}>전체 읽음</button>}
                      </div>
                      {notifications.length === 0 ? (
                        <div style={{ padding:'24px', textAlign:'center', color:'#666', fontSize:'13px' }}>알림이 없습니다.</div>
                      ) : notifications.map(n => (
                        <div key={n.id} onClick={() => markAsRead(n.id)} style={{ padding:'12px 16px', borderBottom:'1px solid #222', background: n.is_read ? 'transparent' : '#1e1e2e', cursor:'pointer' }}>
                          <div style={{ color: n.is_read ? '#888' : '#fff', fontSize:'13px', fontWeight: n.is_read ? 'normal' : 'bold', marginBottom:'2px' }}>{n.title}</div>
                          <div style={{ color:'#666', fontSize:'12px' }}>{n.message}</div>
                          <div style={{ color:'#555', fontSize:'11px', marginTop:'4px' }}>{new Date(n.created_at).toLocaleString('ko-KR')}</div>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="user-info" ref={dropRef}>
                  <button className="user-info-btn" onClick={() => setUserDropOpen(v => !v)} title="내 정보">
                    {user.telegram_photo ? <img src={user.telegram_photo} alt="" className="user-avatar" /> : <span className="user-avatar-default">👤</span>}
                    <div className="user-details">
                      <span className="user-name">{user.nickname}</span>
                      <span className="user-level" style={{ color:'#d4af37' }}>{levelIcon} {userLevel}</span>
                    </div>
